...
If you are interested in annotating or watermarking a GIF animation, I suggest you first look through this document, then jump to [Annotating GIF Animations](../anim_mods/#annotating), to start you off.

------------------------------------------------------------------------

## Annotating Images {#annotating}

The basic problem with labeling an image is doing so such that the text is readable no matter what the image.
The following shows many methods, some of which can be expanded to do more complex tasks.
...
**Splice and Draw** is a very simple way of adding extra space to an image to allow us to draw/annotate the label into the image.
...
**Label using Montage** The montage command in ImageMagick is often overlooked by users as only useful for creating a display of a whole directory of images.
...
**Outlined Label**: The simplest method is to draw the string with an outline to separate the text from the image.
...
The better way to draw a font with a background outline is to draw the text twice.
...
As you can see, it is a lot simpler than drawing the dimmed box yourself, though an extra space at the start and end of the drawn text is recommended, to pad out the box slightly.
...
The dimmed box can be sized to fit the label, and it can be controlled with "`-gravity`" to position it correctly, without needing any specific knowledge of the image it is being added to, or of the font being used.
  
Also, you are not limited to using just a simple dimmed box.
Instead, you can prepare a very complex font image, either before-hand, so you can apply it many times, or on-the-fly on a per image basis.
...
The "`-geometry +0-3`" offset in the above is used to position the composite overlay closer to the edge, as the soft fuzzy outline of this image is often larger than is strictly necessary.
...
The "`composite`" command and the "`-composite`" image operator in ImageMagick provide the primary means to place images on top of other images in various ways.
...
The geometry position of the image is also affected by "`-gravity`", so the positioning of the overlaid image can be defined relative to any of nine (9) different locations.
See "[Positioning Images and Text](#gravity)" below.

On top of "`-geometry`" of the compose overlay, individual images can also have a page or canvas information (set using "`-page`" and "`-repage`" options), that can affect the final position of the images.
This image specific information is however not affected by "`-gravity`".
...
It is a bit like using the overlay as an ink stamp and that is a good way of picturing this operation.
...
Bumpmaps can also be used to set an overall effect on an image, in this case we tile a light colored background pattern over our dragon.
...
The basic goals of watermarking are:
...
-   It shouldn't be too annoying to viewers.
...
In the above I added the "eyes" image so it looks almost like it is part of the wizard's hat - you don't want it to become too integrated into the image either.
...
But by doing some preparation, an image with a transparent background can be created that will be less invasive.
For details of the steps being used to generate the watermark text, see [Font Masking](../fonts/#mask).
Also the mask examples of [Masking Images](../masking/#masks) may be useful for your understanding.
...
As you can see, the watermark is not as bold as before, and even uses shades of grey rather than pure white and black.
...
This works very well, but parts of the watermark will disappear on images with pure white and black pixels.
...
If this is not done the result would be very bold indeed.
...
**Greyed Dissolve:** The same preprocessing technique can also be useful with the dissolve method so the white parts of the watermark image darken slightly on white background, and ditto also to lighten black areas of the watermark on black parts of the image.
...
-   Any image list or layer operators, such as "`-mosaic`", "`-flatten`" and most "`-layers`" methods, and especially not in GIF animations.
    All of these operations use image offsets on a larger virtual canvas (set using the "`-page`", "`-repage`" meta-data settings) to position images.
    Such offsets are always relative to the top-left corner of the image's virtual canvas.
...
Firstly, and most importantly, it defines the origin point that "`-geometry`" uses to position overlay text and images relative to the image's edges, sides and center, without the user needing to know the actual size of the image.
...
There is a push to separate the two aspects such that if a "`justification`" setting is undefined it falls back to using the current "`-gravity`" setting.
...
Not only that, but the direction of the position is also modified so that the direction is inward.

For example, "`-gravity South -geometry +10+10`" will move the label image further into the background.
...
[![\[IM Output\]](gravity_default_pos.jpg)](gravity_default_pos.jpg)
[![\[IM Output\]](gravity_northeast_pos.jpg)](gravity_northeast_pos.jpg)
[![\[IM Output\]](gravity_south_pos.jpg)](gravity_south_pos.jpg)

You can also use "`-gravity`" with "`-draw image`", to draw multiple images with a single command.
...
For more details about the "`-compose`" settings used above, see [Alpha Composition](../compose/).
For other methods of combining and overlaying multiple images into one single image, see the IM Examples section [Layers of Multiple Images](../layers/).
...
As mentioned above, gravity will also affect the positioning of text using either the "`-draw`" '`text`' method, or the better "`-annotate`" text drawing operator.
...
For example, let's actually do this...
...
Let's apply a text offset and draw both the default '`None`' and '`NorthWest`' arguments for "`-gravity`", just so you can see how closely the two forms are related.
...
That is, the two strings are not properly separated by 'pointsize' units, but only by the font's baseline height.
...
Unfortunately this handle is set by gravity BEFORE the text is rotated, and as such does not work very well, unless you use restrict yourself to 'centered text'.
  
For example, here is a typical 'first try' to position text so that it is along the center of the left edge of the image.
Of course it fails rather unexpectedly.
...
There are some animated demos of gravity effects on rotated text written in PerlMagick API (Download "`im_annotation.pl`".
I also created a shell script version of the same program, "`im_annotation`", and "`im_annotation_2`".
  
A trick to making this work is to rotate the whole image first then use center south!
...
This has the advantage that you can arrange to position text without gravity effects, while still using gravity to 'justify' the positioning 'handle' within the text.

In these examples, I added some extra construction lines (which are also not gravity affected) to show how the position is applied from the center point of the image.
...
As you can see, both produce effectively the same result.
...
Having said that, let's look at what happens if you rotate some text using the two different positions.
...
Note how the order of these [Drawing Surface Warps](../draw/#transform) are reversed to the order they are given.
The rotation is performed first, and the translation is performed second.
If you reverse the 'rotate' and 'translate' methods you will get the same result as an ordinary 'text offset', a rotated offset.

The "`-annotate`" operator was designed to make positioning rotated text easier, by specifically asking IM to draw the text with rotation, instead of 'doing surface warping'.
...
Using [SRT Distortion](../distort/#srt) with [Layering Images](../layers/#flatten), is a particularly good method for placing images (or text in images).

Basically, it allows you complete low level control over both the point at which the image is placed, as well as how the image is to be arranged relative to that point.
...
Let's rotate it using distort (layers variant) -- note the use of parentheses to limit which image we distort!
...
All that happened was that the distort rotated the text around the center point (the handle), but so that relative to the 'virtual canvas' that point did not move.
...
So let's place the center at '`+60+60`' in the background image
...
The handle of the text to rotate around and position will this time be at the center bottom of the text, before it was rotated.
...
This technique is looked at more closely in [Extract Information from Other Images](../transform/#fx_other).
...
So any mathematics must be done beforehand.
