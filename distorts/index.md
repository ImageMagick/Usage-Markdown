...
Where "`Xs,Ys`" are source image coordinates and "`Xd,Yd`" are destination image coordinates.
Internally ImageMagick Distort will reverse the above equations so as to do the appropriate [Reverse Pixel Mapping](#mapping) to map "`Xd,Yd`" coordinates to lookup the color at "`Xs,Ys`" in the source image.
...
This results in the areas of the squares becomming smaller, and thus giving the top-right corner a realistic 'in the distance' look.

Bilinear on the other hand does not make one side of the image look 'further away', nor does it try to keep lines straight.
What it tries to do is keep all the spacings between the lines constant, but this results in the diagonal line becomming curved.
...
That is because two neighbouring 'quadrilaterals' even though they may be distorted very differently will still correctly line up correctly edge-to-edge.
...
The effect does produce interesting tile patterns that seem to generate curved 3-dimentional looking surfaces.
...
Without the additional 0.5, the values would be in interger 'pixel coordinates'.
...
However for this specific example, where the coordinaites are close to the expected distorted result, it should be reasonably close.
...
This is to provide an ideal 'reverse' for a full image '`DePolar`' distortion whcih we look at next.
...
Essentually a curved surface wrapping itself around 'point' projection source (a pinhole).
...
What may not be so noticable is that the parts on the far-side of the control points are also compressed, so that as you get further away, the control points have less influence on the result.
