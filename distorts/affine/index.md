...
However for now just note that this is the real meaning of a Affine transformation.
...
they basically tell IM to add a alpha channel and make all pixels outside the input image transparent.
...
For example if you save a image with a negative value into a GIF format image the negative values are replace by zero.
...
For example lets put the koala's nose of the last example near the bottom of the image.
