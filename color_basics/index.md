...
The latter is why we see only grey colors at night.

For more info see [Wikipedia, Cone Cells](http://en.wikipedia.org/wiki/Cone_cell), the graph shows the response of a typical human eye to different wavelengths of light.

Because of this, we only sense the world in terms of Red, Green and Blue electromagnetic wavelengths, and this is why images and image processing is generally all about Red, Green, and Blue, or RGB.

![IM Diagram](../img_diagrams/human_eye_response.gif)

However it isn't quite as simple as that either.
Each of our color sensors actually responds to a range of wavelengths.
...
Rainbows would, in fact, have 'gaps' in them.
...
Note we actually have a fourth vision sensor, but it is not color, but a low light detector, which is used for night vision, when cone sensors no longer work very well.
...
Also some animals see movement far better than any specific color (such as a bull, which, contrary to popular belief, can not see red, but blue-violet and green wavelenths).
As such, a monitor's 50/60Hz update cycle may look like a blaze of movement rather then some sensible image!
...
Images can thus be stored as arrays of 3 values, with each of the three values forming a single pixel, or point of color, to be displayed.
...
In other words, a cyan mask is the exact negative of the amount of red light we want the paper to reflect.
...
To prevent it being confused with blue (which is referred to as 'B'), the black ink (or channel) is assigned the letter 'K'.
...
Note how the amount of cyan, magenta, and yellow has been reduced and is now darker than in CMY colorspace, as they were replaced by an appropriate amount of black.
...
This means far less ink, with the paper becoming less 'wet' and less likely to run or smudge, which is especially good for printers.
...
How you interpret these values is variable as they could represent many different things.
Typically such an image is considered to be Linear-Grayscale and similar to linear RGB, (as opposed to non-linear sRGB).

IM version 6 actually does not have such a color space, and only fakes it with linear RGB color space, setting all three RGB values the same.
...
> The above changes to the 'colorspace' of an image are only a gross arrangement of the colors of an image within memory.
> It also provides very basic (simplistic) color conversions between different colorspaces.
>
> For exact color specifications and color conversions [Color Profiles](../formats/#color_profile) should be used instead, but that only works when using image file formats that can handle color profiles.
...
For example, let's generate and save an image with a simple, linear sequence of gray values...
...
So let's apply it and see the result of a 'gamma corrected image'..
...
So let's save our linear gradient in an sRGB corrected colorspace.
...
However, as the sRGB colors 'white-black' are being requested, the linear data gradient will be automatically converted to sRGB colorspace, producing non-linear data values, and a perceptually linear gradient.
...
Note that the above (or any other gradient) is not a perfect, as perfection is simply impossible, everyone sees things in their own way with slight differences.
Simply put: *"what you see, is never exactly what other people see"*.
...
A more complete and accurate 'human luminance response was developed, and can be seen on line in the thesis [Human Vision, Just Noticeable Difference](http://www.cg.tuwien.ac.at/research/theses/matkovic/node16.html).
...
All these formats (except GIF) allow the use of [Color Profiles](../formats/#color_profile) to definitively specify the colorspace of the image.
...
The biggest differences between sRGB and Gamma image are in extremely dark pictures.
For an 8-bit Gray value of 1, sRGB is 60 times brighter than the equivalent Gamma.
...
Most image processing operators do not care what colorspace an image is using, they just apply their operations to the channel data regardless of its colorspace.
...
As such, doing image processing in a different color space can generate improved results over and above what you would obtain if you directly applied the operation in the normal linear RGB colorspace (or worse sRGB colorspace).

This is especially true of [Color Quantization](../quantize/#quantize) (color reduction), but also in [Image Resizing](../resize/) and, more generally, [Image Distorting](../distorts/).

> ![](../img_www/reminder.gif)![](../img_www/space.gif)
> Cristy, the ImageMagick Coordinator, recommends '`YIQ`' colorspace for color quantization.
> Basically, as this color space emphasizes mid-tones and pastel colors, it is more suitable for skin-tones, rather than the less common primary colors.
...
Stand back from your monitor a few meters (yards) and look at this image.
...
Here are some similar images with different 'gamma' levels so you can see how close your monitor is to a correct '2.2' gamma display for human perception.
...
My computer monitor provided by work is extremely poor.
...
On the other hand my personal laptop has a very uniform display, with a reasonable 2.0 gamma setting according to the above.
...
Many colors have been given specific names, which make them easier to use.
For example, "`RoyalBlue`" is a very nice bright off-blue color.

[![\[IM Output\]](named_colors.png)](named_colors.png)

Here is an image containing all the named colors, including those with numbers, that are available in ImageMagick.
...
>
> To be even more correct, Hexagonal Pyramids should also have been used instead of Cones, though that is much harder to calculate, for little gain.
...
While many colors have names, most colors found in images do not, they are just a set of values, usually 3, that specify a color.
However, three values on their own do not fully define a color, you also need to specify the 'colorspace' or 'color system' that those values belong to.
...
You can directly specify semi-transparent colors in only two different ways.
...
I have displayed the generated color images on a background pattern so that you can see that pattern through the image transparency.

~~~
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'#00ff00ff' -flatten color_hex_1.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'#00ff00c0' -flatten color_hex_2.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'#00ff0090' -flatten color_hex_3.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'#00ff0060' -flatten color_hex_4.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'#00ff0030' -flatten color_hex_5.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'#00ff0000' -flatten color_hex_6.png
~~~
...
> ![](../img_www/warning.gif)![](../img_www/space.gif)
> Before IM v6.3.0, the last set of hex digits contained the color's transparency in the form of a 'matte' or 'opacity' value.
...
> In other words '`FF`' now represents fully-opaque and '`00`' is fully transparent.

You can also specify colors using the special '`rgba()`' color function - where RGB values go from 0 to 255, and the alpha channel is specified as a decimal fraction between 0.0 (transparent) to 1.0 (opaque).

~~~
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'rgba(255,0,0, 1.0)' -flatten color_rgba_1.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'rgba(255,0,0, 0.8)' -flatten color_rgba_2.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'rgba(255,0,0, 0.6)' -flatten color_rgba_3.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'rgba(255,0,0, 0.4)' -flatten color_rgba_4.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'rgba(255,0,0, 0.2)' -flatten color_rgba_5.png
convert -size 60x60 pattern:checkerboard -threshold 50% xc:'rgba(255,0,0, 0.0)' -flatten color_rgba_6.png
~~~
...
You also have the added complication that you must [Set the Alpha Channel](../masking/#alpha_set), before you can actually set the color's transparency.
...
If you would like to see this, make a request on the [IM Developers Forum](../forum_link.cgi?f=2).
...
> ![](../img_www/reminder.gif)![](../img_www/space.gif)
> Note that a fully-transparent color, while completely invisible, still has a color.
...
The primary purpose of the "`-colorspace`" operator is to change the way IM stores the colors of an image within memory.
...
Note that K is short for "blacK", which are the negated intensity values of the image.
...
In a similar way, the channel names '`Alpha`' ('`A`'), '`Opacity`' ('`O`'), and '`Matte`', are all aliases for the "`-channel`" setting referring to the image's transparency information.
...
Whether an operator treats the internal matte channel data as a 'matte' or an 'alpha' value depends on the operator.
...
There is one other method of controlling the colorspace of the stored image data.

The "`-set colorspace`", added in IM v6.4.3-7, will change just the in-memory 'colorspace' setting.
That is, it can convert an RGB image into an HSL image but without changing or modifying the actual pixel data that the image is using.
...
So let's look at how we can manipulate color channels.
Remember each channel is just an array of values.
All the channels would then combine together to represent the actual color of each pixel within the image.

### Separating Channel Images {#separate}

The easiest way of separating out the individual color channels is to use the "`-separate`" operator to extract the current contents of each channel as a gray-scale image.
...
> Very confusing!
>
> In IM v6, the "[`-separate`](../option_link.cgi?separate)", was created to 'separate' these two very different different tasks.
...
If you want to always generate all channels that are present, you can use a "`-channel ALL`" channel setting, or use '`RGBA`' or '`CMYKA`' "[`-channel`](../option_link.cgi?channel)" setting.
...
For example, here we extract the image's grayscale brightness or intensity from the rose image, using a number of different representations.
...
Rec709Luma (from grayscale)
  
[![\[IM Output\]](channel_luma601.gif)](channel_luma601.gif)  
Rec601Luma (from grayscale)
...
Brightness (B from HSB)
  
[![\[IM Output\]](channel_black.gif)](channel_black.gif)  
Neg Black (from CMYK)
  
[![\[IM Output\]](channel_lab_light.gif)](channel_lab_light.gif)  
Luminance (L from Lab)

For the actual formulae, see the official reference to the "`-colorspace`" option.

Note that, as of IM v6.7.7, grayscale images are stored without gamma or sRGB modifications, both in memory and when saved.
...
Note the 'Lightness\*' channel from the Lab (and also LUV) colorspace (not to be confused with 'Lightness' from HSL) is thought to be best matched to the human visual perception, though it is not commonly used for generating grayscale images.
...
One method is to copy one channel to all the other channels, to generate a gray-scale image, just as what the [Separate Operator](#separate) generates.
...
Once you have separated out all the image color channels, and processed them, you will also need to be able to rejoin the channels back together again.
...
These then "`-combine`" to create an image that is declared as being an sRGB colorspace image.
...
This basically changes the colorspace of an image in memory but without mapping the image's pixel data, leaving it as is.
...
This method also works for CMYK images, which are often difficult to handle due to the need for a fourth color channel.
...
> I regarded this as a bug, but it is currently unlikely to be fixed.
...
When finished, I re-combine to build an sRGB image again.
...
You can replace the "[`-negate`](../option_link.cgi?negate)" with your own set of operations to adjust an image's brightness levels.
...
~~~{.skip}
      ...
      # Gamma which is a misuse of the operator, but works VERY well!
...
*Can you think of other ways or zeroing (or maximizing) a color channel which I have not listed above? -- mail me*
...
That is because these are the colorspaces that are typically used for display, printing and traditional storage of images in files.

But while these colorspaces are practical, they are not representative of how we, as humans, actually view the world.
...
Because of this, many colorspaces and color systems have been developed, often from completely independent requirements.
Painters, for example, developed a system of colors (based on color sources like lapus luzuli), shades and tinting.
Later computer systems using RGB, needed better ways for users to select or modify colors, in ways that were not too computationally intensive.
...
Basically, the RGB color cube was rotated in 3 dimensions so that the black-gray-white diagonal axis of the cube became one axis of the colorspace.
And this specified how dark or light a color was.

The key feature of this change was a simple conversion from RGB values that spaced the primary colors equally around this axis, so as to form a Hue that cycles from red, through green, then blue and back to red.

How far away the color was from this axis (radially) was known as saturation or chroma.

For example, let's [Separate](#separate) the channels of the builtin '`rose:`' image after transforming it into the '`HSB`' (Hue, Saturation, Brilliance, also known as HSV, with V for Value), colorspace.
...
Note how the 'Hue' channel image in both colorspaces is the same mottle of almost pure black and white colors.
...
The above raw separations of the color from an image are still difficult to understand.
To better understand the colorspace, we need to try and view it.
The colorspace is more usually represented as a circular polar gradient, showing some part of the colorspace.

You can generate the separate channel value images, and [Combine](#combine) them to generate specific types of images which are hard to generate in other ways.
For example, here we generate a perfect '`HSL`' color wheel.
...
~~~{.hide}
# Create "display-only" versions with labels and borders
convert angular.png         -gravity center -bordercolor black -border 1 label:"Hue"            -append -border 1 do-angular.png
convert solid.png           -gravity center -bordercolor black -border 1 label:"Sat"            -append -border 1 do-solid.png
convert radial.png          -gravity center -bordercolor black -border 1 label:"Lightness"      -append -border 1 do-radial.png
convert colorwheel_HSL.png  -gravity center -bordercolor black -border 1 label:"HSL Colorwheel" -append -border 1 do-colorwheel_HSL.png
~~~

[![\[IM Output\]](do-angular.png)](angular.png) [![\[IM Output\]](do-solid.png)](solid.png) [![\[IM Output\]](do-radial.png)](radial.png) ![](../img_www/right.gif) [![\[IM Output\]](do-colorwheel_HSL.png)](colorwheel_HSL.png)  
...
> It is however the values in the input images that are important here, not the viewing colorspace.
>
...
Actually, the lightness/brilliance channels of HSB and HSL colorspaces are not very useful either, as the various Hues are not handled equally.
Basically it 'equalizes' the intensity of primary colors.
For example, rotating a 'yellow' hue to become a 'blue' hue for example will make a very bright color, very dark, and visa-versa.

For more details see [HSL Disadvantages](http://en.wikipedia.org/wiki/HSL_and_HSV#Disadvantages).
Caution is advised.
  
There are a number of colorpsaces that also use this same 'hex cone' based, hue system - HSB, HCL and HCLp (perceptual HCL) .
...
[![\[IM Output\]](colorwheel_HSL.png)](colorwheel_HSL.png)  [![\[IM Output\]](colorwheel_HSB.png)](colorwheel_HSB.png)  [![\[IM Output\]](colorwheel_HCL.png)](colorwheel_HCL.png)  [![\[IM Output\]](colorwheel_HCLp.png)](colorwheel_HCLp.png)  

Remember all colors shown in the above are generated at a maximum color saturation.
However the '`HSB`' colorspace will produce primary colors at maximum lightness (HSL generated these at half intensity).
...
As a result, when using the '`HCL`' colorspace, the primary colors are located at different intensity levels, with blue closer toward the central black color, and red colors much brighter and further out.
...
That is, a small change in color in one part of the colorspace looks to be about the same, as a similar change in another part of the colorspace.
...
[![\[IM Output\]](rose.gif)](rose.gif) ![==&gt;](../img_www/right.gif) [![\[IM Output\]](separate_lab_luv.png)](separate_lab_luv.png)

The top row of grayscale images are the individual components of the LAB image, and the bottom row are those of the LUV image.

Other better examples of the '`Lab`' and '`Luv`' colorspace can be seen using its cylindrical '`LCHab`' '`LCHvu`' variation, in [The LCH Color Wheel](#colorwheel_LCH) below.

For practical examples of using these colorspaces, see [Resizing in Lab colorspace](../resize/#resize_lab).
...
For completeness, here are the cylindrical representations of the '`Lab`' and '`Luv`' colorspaces, which are known as '`LCHab`' and '`LCHuv`' respectively.
...
[![\[IM Output\]](radial.png)](radial.png) [![\[IM Output\]](solid.png)](solid.png) [![\[IM Output\]](angular.png)](angular.png)  ![==&gt;](../img_www/right.gif) [![\[IM Output\]](colorwheel_LCHab.png)](colorwheel_LCHab.png) [![\[IM Output\]](colorwheel_LCHuv.png)](colorwheel_LCHuv.png)  
...
Even so, GIF images do not allow the use of semi-transparency, so replacing colors in this way is a good method for controlling GIF background transparency (See [GIFs on a Background Pattern](../formats/#bg_pattern) for examples)
...
This is a shortcoming that may change in a future version of IM.

With that caveat, let's look at the ways IM does provide for the direct replacement of specific colors with another color.
...
This may not seem like much, but when you combine it with a [Fuzz Factor](#fuzz) (see below), it becomes a very powerful tool.
...
> ![](../img_www/reminder.gif)![](../img_www/space.gif)
> Be warned that as all matching colors (especially 'fuzzy matched colors', see below) are replaced with a single uniform color, you will not get any anti-aliasing of the edges of the colored areas.
> And you will lose any and all shadow or other shading effects that may be present.
> This can have a seriously detrimental effect to the look of any non-simple non-cartoon like images.
>
...
Transparency also presents no problem, though some internal parts of the image were also made transparent just as they became red in the above...
...
That is, you can make all matching colors transparent, or semi-transparent, without actually changing the color of the pixel itself - with the appropriate file format, of course.
...
The [Draw Color](../draw/#color) methods also provide you with a way of replacing a color by 'floodfilling'.
...
The specified point will not only provide the stating (seed) point, but also the color that you are trying to replace.
...
Note that the white areas which are not 'attached' to the 0,0 white pixel were not floodfilled with red.
...
This can be regarded as the operator's feature as well as its curse.

**A small [Fuzz Factor](#fuzz) is recommended for flood filling.
Or ensure seed point exactly matches sought color.**
...
This will replace any color that is 'white' with 'red' that is directly part of the area surrounding the seed pixel starting at +0+0, which is guaranteed to be 'white' due to the added border.
...
The overall results of just selecting a single color to replace, as shown in the previous examples, are usually not very nice.
The edges of areas of solid colors generally have a mix of colors, due to anti-aliasing (See [Anti-Aliasing](../antialiasing/) for more information).
...
Well, okay let's try that in plain English.
You have a specific color.
Another color will be treated as being the *same* as the color being looked for, if the difference between these colors is less than the fuzz factor setting.
As you increase the fuzz factor, so colours successively further away from the original colour will be affected.

So let's try that on our cow image so as to convert not only pure-black but also near-black colors to red.
...
As you can see, we now replaced all the 'dark' pixels of the image with red.
...
The "`-fuzz`" operator affects just about any operator which compares specific colors within an image.
This includes: "`-opaque`", "`-transparent`", "`-floodfill`", "`-trim`", "`-deconstruct`", "`-draw 'color'`", "`-draw 'matte'`", and probably others.
It also affects GIF "`-layers OptimizeTransparency`", and "`-compose ChangeMask`" handling.

It also affects the results of "`compare`" and specifically the "`-metric AE`" or Absolute Error Pixel Count.
...
As you can see from the above formulae, direct color distances are definitely not a nice way of setting the fuzz factor to use, as it is also dependant on exactly what compile time [Quality Setting](../basics/#quality) is used.
...
To demonstrate color distances more, let's use a progressively larger fuzz factor percentage around the blue colors...
...
By 51% all the colors in the image have matched the near-perfect gray.
...
In later images I make the color being matched more transparent until it is fully-transparent, however the [Fuzz Factor](#fuzz) remains a constant 20%.
...
If you want to match all colors regardless of their transparency, then you will need to [Turn Off Transparent Channel](../masking/#alpha_off) of the image, at least temporarily.
...
However, as the color being matched gets more semi-transparent, the number of matching semi-transparent colors will seem to become larger, until a fully-transparent grey will match any near-transparent color.
...
The other thing to notice is that (as of IM v6.6.6-4) the distance from a fully-transparent color (grey or otherwise) is purely a function of the color's transparency (alpha value).
The last image in the above matched all pixels that were within 20% of being fully-transparent, regardless of actual color.
...
Notice that only the top 5% of the near opaque colors in the above did not match, while all the other semi-transparent colors, were turned grey.
