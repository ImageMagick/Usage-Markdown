# ImageMagick v6 Examples --  
 ![](../img_www/space.gif) Basic Usage

**Index**  
[![](../img_www/granitesm_left.gif) ImageMagick Examples Preface and Index](../)
[![](../img_www/granitesm_right.gif) ImageMagick Command Line Processing](#cmdline)
-   [Why did Command Line Style Change!](#why)
-   [IMv6 command line Syntax](#syntax)
-   [Operators and Setting Options](#options)
    -   [Settings](#option_settings):  [operator](#option_ops),  [input](#option_in),  [output](#option_out),  [control](#option_ctrl)
    -   [Operators](#option_operators):  [create](#option_create),  [simple](#option_simple),  [list](#option_list),  [stack](#option_stack),  [misc](#option_misc)
-   [A Working Example of an IM Command](#example) identify -format "%l" rose.png
...
-   [Conjure -- Experimental IM Scripting Language](#conjure)

[![](../img_www/granitesm_right.gif) Image Lists](#image_list)
...
-   [Image List Operators](#list_ops)
    [-delete](#delete),  [-insert](#insert),  [-swap](#swap),
    [-reverse](#reverse),  [-clone](#clone),  [-duplicate](#duplicate)
...
-   [Page, Repage and Image Offset Control](#page)
...
 
[![](../img_www/granitesm_right.gif) ImageMagick Operational Controls](#controls)
...
Initialize and Create an empty 'image list'
  
empty seq
  
**`eye.gif`**
  
Read in image and add to end of current image list
  
1 image
  
**`news.gif`**
  
Add a second image into list (now with two images)
...
Add another image to the image list
...
As this is last argument, an explict **`-write`** operation is performed with this argument. The single image in the current image the list is written using the given filename and image file format.
...
Alternatively, you can generate the images using a shell script loop, and pipe the result into a final command to merge them into the final image or image sequnece. For example of this see [Layered Images Examples](../layers/#layer_examples), or the various [Warped Image Animations](../warping/#animations) shell script generators.
...
The last few forms will allow up to a maximum of 5 posible input values, and are typlically used for specifying a value for each of the standard RGBKA image channels.
...
There is a [Future Proposal](../bugs/future/#settings) about exactly when a percent escape will be expanded that could fix this problem with geometry arguments. And hopefull will be part of IMv7.
...
Because of their nature, either of last two types of arguments are often pre-processed so as to expand [Image Properity Percent Escape](http://imagemagick.org/script/escape.php) within the string.
...
If the string is not read from a file or input stream, then any '`\n`' strings are replaced with a 'newline' character, and any '`%`' prefixed label is replaced with the appropriate value. See [Image Properity Percent Escapes](http://imagemagick.org/script/escape.php) for the complete list of replacements.
...
If you do want to just simple 'display' the resulting image the special '`show:`' or '`win:`' output [Spawning Delegate](../files/delegate_spawn) will do the same thing by runing the "`display`" command on the output image, and exiting without waiting for that window to be closed.
...
Alternativally, by inserting this line instead, you can display all the current images generated so far at that point in the processing...
...
So how can use you create a label using a image properity or attribute? The "`-define`" option does not currently allow the use of image properities!
...
When writing to a [PNG](../formats/#png_formats) file format setting a "`-type`" of '`Pallette`' will force it to use a color indexed "`PNG8`' internal image format. Simularly using "`BiLevel`" will force IM to dither color images to black and white for most image file formats.
...
What a lot of people refer to as 8 bit images (8 bit/pixel), are really images with an 8 bit palette or color map (giving a maximum 256 color limit over the whole image). The actual pixel data is then a 8-bit index value (0-255) which is then used to look up the color for that pixel from the color table. That is the 'raster' (pixel array) is just an index used to lookup the actual pixel color from a separate table of colors.
...
At Q8 the intermedite images in memory will remain stored as 8 bit quality, and thus multiple operations will each introduce more and more distortion. The result can be serious bit level rounding effects, especially for extreme colors near white and black. (see below).
...
However while a Q8 version of IM it will let you output a 16 bit depth images, such image will still only have information equivelent to 8 bit depth as the quality is just not there to be saved.
...
Also may operators that generate extra colors such as [Image Resizing](../resize/), will also reset the 'depth' of the image in memory to the compile time quality setting, so that IM will then try to save it at the appropriate, and hopefully higher depth, if posible.
...
*Note that Q64 automatically uses "double long floats", rather than 64 bit intergers. It is sort of a double precision HDRI, but while very precise is the slowest of all in-memory quality settings, and most memory intensive.*
...
The image distortion methods available.
  
**morphology**
  
The image morphological methods available.
  
**command**
  
what command line options (both settings and operators) are available
...
When IM outputs floating point values in response to various debugging, verbose, or formating requests, this setting sets how precise you want that output.
...
Report the trace points at the start of each library function
