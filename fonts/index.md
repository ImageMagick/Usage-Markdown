...
> This usage, however, is not recommended as many operators that use the "`-fill`" color will not understand a tiling image, and default to using 'black' instead.
...
Of course the Candice font is not a good font to show this effect, and other details may need to be added to make the result look more 3D like.
...
> :EXPERT:
> Both "`-annotate`" and "`-draw skew?`" operations actually rotate the X and Y axis of the drawing surface.
...
If you swap the two colors you will of course get a raised font, instead of an indented font.

### Extruded {#extrude}

Or 3D block fonts can be generated by repeating the font multiple times.
...
It is also a useful technique for other graphic libraries and programs (Like "`GD`" from "`PHP`", etc) where a "`-stroke`" setting is not available for use.
...
Using the [-stroke](../option_link.cgi?stroke)" setting is taken even further, in [Stroke and StrokeWidth Options](../draw/#stroke) where this aspect of drawing operators is explored in greater depth.
...
However, the last font drawn must be with done with a fill setting of "none", or it will not work.
...
This resulted in an interesting enlargement of the font, with a thick outline.
The 'puffy' looking font is as if it had been inflated like a balloon.
...
A variation on this is to draw each and every character separately so that each character is overlaid on top of the previous characters.
...
Note that, unlike when using the "`-kerning`" setting (previous example), each character's position in the above was adjusted artistically by different amounts rather than just some simple fixed amount.
For example, only a little overlap between the 't' and the 'h', but a lot more overlap between the 'n' and 'y' characters.
...
This operator allows you to take an image and spread it out in all directions.
...
If you want to use a blur on an existing image, you will have to draw the font separately (on a transparent background), then overlay it on the background image.
...
Use the fuzzy font as an offset for a soft shadow.
...
The "`-shadow`" operator will not only allow you to generate and position soft fuzzy shadows for images containing transparency, but will also allow you to use any color and set a general transparency level.
...
This effectively moves the 50% blur point further away from the edge of the font.
It will also allow an even larger blur value to be used, allowing the black color to spread out further.
...
With the introduction of [Variable Blur Mapping](../mapping/#blur) you can now blur a shadow so that it becomes more blurry with the apparent distance of that shadow from the casting object.
...
A better method is to use a three point [Affine Distortion](../distorts/#affine) which gives you better control over the placement of the shadow (See [3d Shadows, using Affine Shears](../distorts/#shadow3d)).
Of course you will still need the [Variable Blur](../mapping/#blur) technique to make it look right.
  
### Beveled Font {#bevel}

The [Shade Operator](../transform/#shade) can be used to generate a very nice looking 3D font with a beveled and smoothly curving edge.
...
The result looks like the sharp ridges are pushing up into a rubber sheet.
...
One of the specialized blur operators, "`-motion-blur`" allows you to create a comet-like tail to objects in an image.
...
Do you have an interesting font transform to add to the above list?

------------------------------------------------------------------------

## Using a Mask Image with Fonts {#mask}

Drawing a "Stamped Font" on a background image is actually a lot more difficult than most of the methods of generating fancy fonts I detailed above.
...
Well, our image above is almost right, so let's just mask out all the parts we don't want.
...
These grey pixels make the final image edges smoother, and are very important, as these pixels represent semi-transparent (only half visible) pixels.
...
It would be better if, we only needed one image, with the mask built directly into the the image itself.
