...
Thus each of the red, green and blue 'channels' contain a set of 'intensity' or 'grayscale' values.
...
This follows from the principal that "any well-behaved function can be represented by a superposition (combination or sum) of sinusoidal waves".
...
To allow for proper centering of the 'FFT origin' in the center of the image, it is also forced to have a even (multiple of 2) dimensions.
...
As such this representation can not be using in a normal version of ImageMagick, which would clip such images (see example below of the resulting effects.
...
Now, lets simply try a Fourier Transform round trip on the Lena image.
...
Both images generated by the FFT process are very sensitive to modification, where even small changes can result greatly distorted results.
...
Such an image isn't really very interesting to look at to study, so lets enhance the result with a log transform to produce the a '*frequency spectrum*' image.
...
You will see that the HDRI version of IM produced a much more accurate result, at roughly the same speed as before (speed may vary depending on your computer).
Though it will have required much more memory than a a normal Q16 IM (See [Compile-Time Quality](../basics/#quality)).
...
If you don't the resulting image with have a 'dirty' look about them due to the 'clipping' of negative values.
For example...
...
The other thing to remember is that whichever form of FFT images you generate will also effect ALL image processing operations you want to apply to the FFT images.
...
The biggest difference between the two is that the special DC or 'average color' has no imaginary component and as such is only presetn in the magnitude image.

The diagonal mirror effect you see in both images is caused by the loss of the 'sign' information contained in the othe component.
...
it does not care!
To a fourier transform a image is just an array of values, and that is all.
...
This is just as if we really were really adding some constant to each pixel in the original image.
...
> A FFT transform of such a im-perfect tile, will result in an array of undesired harmonics, rather than single 'dots' in the Fourier Transform Spectrum.
>
...
The smaller the wave length the higher the frequency, so the further the dots will be from the DC value.

In fact by dividing size of the image by the frequency (distance of the dots from the center), will give you the wavelength (distance between peaks) of the wave.
...
A pattern of small features on the original image require small wave lengths, and thus large frequencies.
...
This duality of positive and negative frequencies in FFT images explains why all FFT image spectrum's (such as the Lena spectrum repeated left) is always symmetrical about the center.
...
> The other half is generated by rotations and duplication of the generated data.
>
...
You can also see that the rectangle appeared to be rotated 90 degrees.
...
Now, lets rotate the rectangle by 45 degrees.
...
That is the effect of some rotated object will also be rotated in its Fourier Transform.
...
These artifacts occurs due to the small size of the circle.
...
Other than the noise produced by the rectangular array of the pattern, the result was that a Gaussian pattern produced a almost identical Gaussian frequency pattern.
...
So lets first increase the contrast on the Lena image using an exponent of 0.9 and then decrease the contrast using an exponent of 1.1.
...
The simplest filter is just a an equally-weighted, square array.
...
Alternately, one can use a filter such as a Gaussian shape that is already by definition tapered.
...
Lets try this on the [clown image](http://www.mediacy.com/index.aspx?page=AH_FFTExample), which contains a diagonally striped dither-like pattern.
...
Lets try this on an another example.
...
As a exercise, try removing the string from the image.
