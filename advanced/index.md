...
[![\[IM Output\]](pokemon.gif)](pokemon.gif) So let's start with the reflection of a relatively simple image, in this case a image of a Pokemon character, Azumarill (See right).
...
So let use try this again, but this time let's color the reflection by the color of the reflecting surface.
...
For example, let's generate a horizontal wooden floor, using a infinite tiling technique from [Viewing Distant Horizons](../distorts/#horizon).
...
First let's create a image to use, in this case a curvy letter A.
